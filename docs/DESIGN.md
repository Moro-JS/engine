...
- **M5 — hardening** ✅: libFuzzer harnesses + seed corpus for both parsers (`test/fuzz/`), nightly CI fuzz + sanitizer jobs, ASan/UBSan clean, idle/ slowloris timeout, functional load soak (no crash/hang/leak), `SECURITY.md` + `docs/THREAT_MODEL.md`. Found+fixed a Content-Length overflow smuggling bug during the threat-model pass. External security review is recommended before untrusted-facing TLS production (a human gate, not self-certifiable).
