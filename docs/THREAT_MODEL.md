...
- **TB0 — Network → libuv/socket engine (`src/server.h`).** The attacker controls every byte, how it is fragmented across TCP segments, and its timing. The socket engine accepts connections, feeds bytes to the parser incrementally, writes responses in request order, and enforces connection lifecycle/timeouts.
  On Linux 6.1+ the same engine may run on the **io_uring transport** (`src/uring.h`) instead of libuv streams — opt-in (`MORO_ENGINE_TRANSPORT=uring`), gated by a feature probe and self-test, never required, and identical above the transport seam. Two transport-specific properties matter to a reviewer: (1) receive buffers are **kernel-provided** (a registered provided-buffer ring, 16 KiB × 512 per loop thread): the kernel writes a segment into a buffer it chose, the engine parses straight from it and recycles the buffer immediately after dispatch — no code path retains a pointer into a receive buffer (the parser copies partial heads/bodies into its own storage, TLS copies into the memory BIO, WebSocket unmasks in place then copies), which the transport-parity and conformance suites exercise under both transports; (2) teardown is **shutdown, cancel, close**: a synchronous `shutdown(SHUT_WR)` puts the FIN on the wire, then one submission carries `IORING_ASYNC_CANCEL` by fd (resolving the recv) and `IORING_OP_CLOSE`, and a `Connection` is freed only when its close state is reached *and* every submitted SQE that names it has completed (`inflight == 0`), so a late completion can never touch freed memory. The ring's SQ/CQ are private anonymous mappings; a peer cannot influence which buffer is selected or observe another connection's data.
...
- **Deterministic cleanup on close.** `doClose` erases the connection from the `globalRequests`/`globalWebSockets` registries and the live-connection set, stops reads, and frees the `Connection` (and its `WsParser`) in the libuv close callback — or, on the io_uring transport, once the cancel and close completions have been reaped and no submitted operation references it. `Server::close()` iterates a snapshot of live connections and closes each, so no keep-alive socket keeps the loop alive after shutdown. A ring that stops accepting submissions at runtime is treated as dead: every connection on it is aborted through the normal abort path (callbacks fire once) and later servers on that thread fall back to libuv.

> **Hardened (v1.0):** bytes received while a response is in flight are now bounded by a per-connection `maxPendingBytes` cap (default `maxHeadSize + maxBodySize`; the connection is hard-closed on overflow), and a `maxConnections` cap drops accepts beyond the limit (0 = unlimited, matching Node's `http.Server` default). WebSocket sends are bounded by a 1 MiB write-queue cap (the slow consumer is shed with a 1013 close once the queue overflows). Per-server JS state (V8 `Global` callback/context handles) and the `Server`/`Connection` C++ objects are freed via deferred cleanup once every libuv handle is reaped — no leak across `serve()`/`close()` cycles. **Since then:** the sweep gained a per-request receive budget (`requestTimeoutMs`, 408 on expiry — a steady byte-trickle is now reaped) and a response-delivery budget (`responseTimeoutMs` — a peer that never reads its response is now reaped), plus an opt-in `responseBackpressureLimit` outbound-queue cap. **Lingering close** (1.1.6): after the last response of a `Connection: close` exchange the engine half-closes and keeps the socket open, discarding input, until the peer's FIN — the RFC 9112 §9.6 staged close that keeps a request the peer had already written from being answered with a RST. A peer that never closes holds that slot for at most `kLingerMs` (2 s, `Server::onSweep`); the input discarded meanwhile is read into the shared receive buffer and dropped, never buffered, so a byte-flood on a lingering connection costs the reads and nothing else. Lingering connections count against `maxConnections`, which is the intended bound for a peer that opens connections faster than it closes them. **Remaining:** `maxConnections` is opt-in (unlimited by default). See [Known limitations](#known-limitations--non-goals).
