...
- **Backpressure.** Streaming `write`/`wsSend` report backpressure against a 256 KiB high-water mark and resume via `onWritable`, so a slow-reading client cannot force unbounded server-side write buffering silently (the app is told to pause). Independently of app cooperation, the `responseTimeoutMs` delivery budget (section 4) sheds any connection whose queued writes make no progress, and the opt-in `responseBackpressureLimit` hard-caps the outbound queue.
