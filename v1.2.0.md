...
- **The FIN travels with the last bytes of a `Connection: close` response**
  (macOS `TCP_NOPUSH` + `MSG_EOF`, Linux `MSG_MORE` + `shutdown`): the peer
  can no longer close first and inherit the TIME_WAIT. On macOS, where a
  closed port stays unusable for 30 s, a one-connection-per-request load
  against the engine decayed to ~5.8k conn/s as the client drained its
  ephemeral ports; it now sustains 27.5k, ahead of Bun and uWebSockets.js on
  the same harness. Linux churn gains the same guarantee.
