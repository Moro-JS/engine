...
The loop also reaps pending task work and posted completions in its prepare
phase, before it blocks, so a wake is a hint rather than a contract: under
`defer-taskrun` nothing the kernel holds for the engine can wait past one
loop turn. The churn soak prints the ring mode it ran under.
